...
export function middleware(request: NextRequest) {
  ...
  // /dev is the development-only component harness, and this is the layer that keeps
  // it out of production with a real status code before any rendering starts.
  //
  // The page already calls notFound() when NODE_ENV is production, and it does stop
  // any specimen markup being served, but it could not set the status while
  // src/app/loading.tsx sat at the app root (it moved on 22 September 2026): Next 14
  // wrapped every route in that Suspense boundary and flushed the shell with a 200
  // before the page component ran. Once the headers are sent the status cannot
  // change, so production answered 200 carrying the loading
