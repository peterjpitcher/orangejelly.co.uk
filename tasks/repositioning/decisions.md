...
| D42 | **OPEN. Seven pages survive launch still publishing a price.** `/ways-to-work` redirects to `/how-we-work` at phase 4, but its four package children have no redirect, and neither do the three `/services` pages. On launch day they are live, indexable, and quoting £375, £900 and £1,800, on a site whose whole argument is that every engagement is scoped to the problem. The positioning gate cannot see them: it only scans the repositioned surfaces, which is why this went unnoticed. **Two possible answers and they are business decisions, not edits: either the packages still exist, in which case D3 is narrower than it reads and those pages need saying so, or they do not, in which case the four package pages and the three service pages retire alongside their parent.** Recommendation: retire them, because a reader who finds £375 from a search result and then reads "there is no price list" on `/start-here` has been told two different things by the same company. Raised with Peter 30 Aug 2026. | 30 Aug 2026 | **Needs Peter** |
