# What we build: approved digital growth offer

Websites, applications and the systems behind them.

Bring a defined project or a customer journey that needs to work better. AI belongs where it performs a useful task, with agreed data and human checks. Using AI during development is different from delivering AI features.

## Hospitality websites

Websites that make it easier for guests to find answers, choose your venue and book with you.

Destination: /solutions/hospitality-websites

## Bespoke applications

Browser applications, customer portals and internal tools built around the way your business works.

Destination: /solutions/bespoke-applications

## Booking systems

Connect your existing booking tools or build the parts your customer journey is missing.

Destination: /solutions/booking-systems

## Supporting capabilities

### Growth and commercial strategy

Where the business is going, and what it stops doing to get there.

### Numbers you can act on

The three numbers that would change a decision, not the ninety that will not.

### What you offer, and how you describe it

What you actually sell, in words a customer would use.

### Marketing and getting found

Making enough of the right people know and care.

### Turning interest into sales

Closing the gap between someone asking and someone buying.

### Websites and booking systems

The place the decision usually gets made, or lost.

### Keeping in touch with customers

Staying useful to people who have already bought, so they come back.

### Pricing and margin

What is left after the sale, and why more of it is not.

### Supplier costs and contracts

The cost line nobody has looked at in three years.

### How the work gets done

Removing the jobs that should not exist.

### Automation and internal tools

Software instead of somebody's Tuesday.

### Where AI would genuinely help

Where it earns its place, and an honest answer where it does not.

### Ways of working the team can follow

Making the improvement survive the person who made it.

## Fit

Work with no agreed measure of success, because there is then no honest way to end it.

Technology chosen before the problem was defined, including ours.

Anything we would be the second-best supplier for. There is usually somebody better and we would rather say who.

Existing published hourly terms remain unchanged. No new support or delivery promise is introduced.
## Amendments, 8 September 2026: search vocabulary

Peter approved a vocabulary pass after the keyword programme found the page never used the words
its customers search with. Brief:
`tasks/keyword-plan/handoffs/2026-09-08-editorial-team-vocabulary.md`. Demand is banded, United
Kingdom, English, measured 8 September 2026: `websites development` 1,000 to 10,000 a month at Low
competition, `website development in uk` 1,000 to 10,000 at Medium, `website development agency uk`
100 to 1,000 at Low.

- **Standfirst, now on the page:** We do website development and build bespoke applications: the
  websites customers see, and the workflows behind them. The aim is a clearer customer experience,
  more bookings and a business that works better. AI is part of the build where it has a useful job
  to do.
- **Title:** Website Development & Bespoke Applications | Orange Jelly
- **Meta description:** Website development, bespoke applications and connected booking systems.
  Practical work that turns customer interest into bookings and repeat business.
- **Build card, Bespoke applications:** "Browser applications" became "Custom web applications".

The eyebrow, the H1, the capability groups and the declined list are untouched.
