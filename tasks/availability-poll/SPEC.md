...
To see, correct or delete your data, write to privacy@orangejelly.co.uk. Please
...
<p style="margin:0 0 8px;font-size:13px;color:rgba(26,47,73,0.72);">
  ...
  <a href="mailto:privacy@orangejelly.co.uk" style="color:#01619E;">privacy@orangejelly.co.uk</a>
...
- **`List-Unsubscribe` on the digest and the nudge.** Both are recurring-ish mail to an inbox and both need a one-click opt-out header, via the `headers` passthrough on `sendPollEmail`. Send both `List-Unsubscribe: <https://www.orangejelly.co.uk/availability/o/<organiser_token>/unsubscribe>, <mailto:privacy@orangejelly.co.uk?subject=unsubscribe>` and `List-Unsubscribe-Post: List-Unsubscribe=One-Click` — Gmail's bulk-sender rules require the POST form, and the header alone without it is not compliant. The unsubscribe route sets a **NET NEW** `polls.digest_opt_out boolean not null default false`, which both §4.2 and §4.5 must check before sending. The verify, links, edit-link and confirm emails are single transactional messages and do not need one. "We're under the volume threshold" is not a defence worth relying on.
...
| `privacy@orangejelly.co.uk` as a real, monitored inbox | prerequisite | 3 |
...
| **Q7** | **`privacy@orangejelly.co.uk` as a real, monitored inbox.** Nothing in the repo references it today. | **Phase 3 — prerequisite.** | The Article 14 notice names it as the route for Article 15–17 rights. Naming an inbox that does not exist is worse than naming none. |
