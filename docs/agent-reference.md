...
Set `status: published` and a `publishedDate`; a future date holds the post until then. Commit as `feat(blog): add guide, <title>`, push, and verify the live URL. The search index (`public/search-index.json`) and the feeds are regenerated at build time and are not committed.
...
| `build:search`, `build:feeds`, `build:all` | Search index to `public/search-index.json`; RSS and JSON feeds; both |
